...
# --- CONFIGURATION ---
# We run for December 2024 AND January 2025 as requested
RUN_RANGES = [
...
GRANULARITY_MIN = 60
TOPIC = 'data'
TABLE_NAME = "asense_table_data"  # New Table Only
...
def scan_data_topic(device_id, start_ms, end_ms, gran_min):
    """Fetches all timestamps for a month using the GSI and maps to blocks."""
    ...
    query_count = 0
    try:
        # One query to rule them all (per month)
        params = {
            'IndexName': GSI_NAME,
        ...
        while True:
            ...
            # Handle pagination if month has > 1MB of timestamps
...
def run_all():
    ...
    for time_range in RUN_RANGES:
        ...
        # Save CSV
        csv_path = f"tests/coverage/report_{run_id_slug}.csv"
        ...
        html_path = f"tests/coverage/heatmap_{TOPIC.upper()}_{run_id_slug}.html"
        fig.write_html(html_path)
        print(f"✅ CSV and Heatmap saved for {TOPIC}")
